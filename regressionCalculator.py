...
n75 = "Entropy_output_of_radius(75.0).csv"
...
ax[0].scatter(distance, entropy, s=10, marker="x")  
ax[0].plot(xSeg, line(xSeg))  
print(np.polyfit(distance, entropy, 4))

xSegReduced = np.linspace(min(distanceReduced), max(distanceReduced), len(distanceReduced)) 


ax[1].scatter(distanceReduced, entropyReduced, s=10, marker="x")  
...
ax[1].plot(xSegReduced, line(xSegReduced))   
...
print(distEnt[0]) 
print(distEnt[-1])
plt.scatter(distanceReduced, entropyReduced) 
plt.show() 
